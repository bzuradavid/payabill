...
export async function ManagerDashboard({
  ...
  const [stats, recentBills, pendingApprovals] = await Promise.all([
    billService.getDashboardStats(),
    billService.getRecentBills(8),
    billService.getPendingApprovals(5),
  ]);
        ...
        <CardHeader>
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Bills submitted by your team awaiting your review
            </p>
          </div>
          <Link
                  ...
                  {pendingApprovals.map((bill) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3.5 text-slate-500">
                        {bill.createdBy.name ?? bill.createdBy.email ?? "—"}
                      </td>
                      <td className="px-6 py-3.5 text-slate-500">
                      ...
                      <td className="px-6 py-3.5 text-right">
                        <Link
                          href={`/bills/${bill.id}`}
                          className="text-xs font-semibold text-[#312D97] hover:underline"
                        >
                          Review →
                        </Link>
                      </td>
